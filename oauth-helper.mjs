...
const port = Number(process.env.ORO_AUTH_HELPER_PORT || process.env.PORT || '18789');
...
function extractAuthUrl(text) {
  ...
  const matches = clean.match(/https:\/\/auth\.openai\.com\/oauth\/authorize\?[^\s]+/g);
  ...
  if (req.method === 'POST' && reqUrl.pathname === '/callback') {
    ...
    req.on('end', () => {
      ...
      if (!/^http:\/\/localhost:1455\/auth\/callback\?/.test(callback) && !/^http:\/\/127\.0\.0\.1:1455\/auth\/callback\?/.test(callback)) {
        ...
        res.end('<meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:system-ui;padding:24px"><h3>콜백 URL 형식이 올바르지 않습니다.</h3><p>localhost:1455 전체 주소를 붙여 넣어주세요.</p></body>');
        return;
      }
      try {
        fs.appendFileSync(fifo, `${callback}\n`, { encoding: 'utf8' });
        res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
        res.end('<meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:system-ui;padding:24px"><h2>전송 완료</h2><p>ChatGPT OAuth 완료를 확인 중입니다. 잠시 후 이전 화면으로 돌아가 상태를 확인하세요.</p></body>');
      } catch (err) {
